...
public class GameManager : MonoBehaviour
{
    ...
    public bool AutoModePcVrDetection = false;
    ...
    private void Awake()
    {
        if (!AutoModePcVrDetection || GameMode == Mode.Flat) return;

        if (XRGeneralSettings.Instance == null || XRGeneralSettings.Instance.Manager.activeLoaders.Count == 0)
        {
            Debug.Log("No XR Loader found, setting GameMode to Flat");
            GameMode = Mode.Immersive;
        }
        else
        {
            Debug.Log("XR Loader found, setting GameMode to VR");
            GameMode = Mode.VR;
        }   
    }
