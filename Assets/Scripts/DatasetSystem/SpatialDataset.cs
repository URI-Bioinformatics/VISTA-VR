        ...
        */
        ...
        if (loadedData.Layers != null)
        {
            foreach (LayerData data in loadedData.Layers) // Assumes Layers are already sorted by index from LoadMetadata
            {
                ...
                BakedDataLayer layerObjScript = bakedLayerObj.GetComponent<BakedDataLayer>();
                if (data.LayerType == 1) layerObjScript.SetLayerColor(Color.white);
                else layerObjScript.SetRandomLayerColor();
